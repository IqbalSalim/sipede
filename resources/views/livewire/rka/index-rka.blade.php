<div>
    <div class="bg-gray-100" x-data="{ modalCreate: false, modalShow: false, modalEdit: false }"
        ...
        <livewire:rka.create-rka></livewire:rka.create-rka>
        <livewire:rka.update-rka></livewire:rka.update-rka>
                                ...
                                <tr>
                                    ...
                                    <td class="px-2 md:px-6">
                                        <div class="flex flex-row items-center space-x-4">
                                            <button wire:click.prevent="$emit('getfilePdf', {{ $rka->id }})"
                                                @click="modalShow = true" type="button"
                                                class="text-xs btn-primary">lihat</button>
                                            <button wire:click.prevent="$emit('getRka', {{ $rka->id }})"
                                                @click="modalEdit = true" type="button"
                                                class="text-xs btn-success">edit</button>
                                            <button wire:click.prevent="alertConfirm({{ $rka->id }})" type="button"
