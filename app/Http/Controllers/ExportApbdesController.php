...
class ExportApbdesController extends Controller
{
    public function exportApbdes(Request $request)
    {
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            foreach ($que as $row) {
                if ($row->kegiatan->subbidang->bidang_id == $i) {
                    ...
                    array_push($rapbdes['b_a_' . $i], $row->kegiatan->nama);
                    array_push($rapbdes['b_b_' . $i], $row->jumlah);
                    array_push($rapbdes['b_c_' . $i], $row->sumber);
                }
            }
        }
        ...
        for ($i = 1; $i <= 3; $i++) {
            ...
            $rapbdes['b_a_' . $i] = new ExcelParam(CellSetterArrayValueSpecial::class, $rapbdes['b_a_' . $i]);
            $rapbdes['b_b_' . $i] = new ExcelParam(CellSetterArrayValueSpecial::class, $rapbdes['b_b_' . $i]);
            $rapbdes['b_c_' . $i] = new ExcelParam(CellSetterArrayValueSpecial::class, $rapbdes['b_c_' . $i]);
        }
